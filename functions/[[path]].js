...
(function(){
  ...
  render_AdminDashboard(dbData, m) {
      ...
      @media (max-width:768px){
        ...
        <script>
          ...
          async function saveSettings(btn){
            ...
            btn.disabled=true;
            try{
              const res=await fetch(ADMIN_PATH+'/api/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});
              if(res.ok){alert('✅ 配置已保存并生效！');location.reload();}
    ...
    let historyTotal = 0;extM_Y}_${String(nextM_M).padStart(2, '0')}`;
    for (const v of statsMap.values()) {
    }onst buildCard = (id, name, emoji, isMini) => {
      const stat = statsMap.get(id) || { total_clicks: 0, last_time: '' };
    let viewTotalDenominator = 0;ap.get(id) || 0;
    if (isDayMode) {TargetVal = periodMap.get(id) || 0;
      for (const c of monthContextMap.values()) viewTotalDenominator += c;
    } else {l2Label, col2Val, col3Label, col3Val, progressVal = 0;
    } if (isDayMode) {
        col2Label = (m === this.time.todayStr) ? "今日" : "当日";
    let prevDay = m, nextDay = m, prevMonthStr = "", nextMonthStr = "";
    try {ol3Label = "当月";
      if (isDayMode) {hContextVal;
        const d = new Date(m);lDenominator > 0 ? ((monthContextVal / viewTotalDenominator) * 100).toFixed(1) : 0;
        d.setDate(d.getDate() - 1);
        prevDay = d.toISOString().split('T')[0];
        d.setDate(d.getDate() + 2);
        nextDay = d.toISOString().split('T')[0];"本月" : "当月";
      } col3Val = selectedTargetVal;
      const currentY_int = parseInt(m.substring(0, 4));ctedTargetVal / viewTotalDenominator) * 100).toFixed(1) : 0;
      const currentM_int = parseInt(m.substring(5, 7));
      let prevM_Y = currentY_int, prevM_M = currentM_int - 1;
      if (prevM_M === 0) {
        prevM_Y -= 1; !== '暂无') {
      ...
      if (nextM_M === 13) {
        nextM_Y += 1;
      }onst safeId = this.escapeAttr(id || '');
      nextMonthStr = `${nextM_Y}_${String(nextM_M).padStart(2, '0')}`;
    } catch (e) {}me = this.escapeHtml(name || '');
      const encodedName = encodeURIComponent(name || '');
      const stat = statsMap.get(id) || { total_clicks: 0, last_time: '' };
      const realTodayVal = dailyMap.get(id) || 0;penLog('${safeId}','${safeM}','${encodedName}')">
      const selectedTargetVal = periodMap.get(id) || 0;
      const monthContextVal = monthContextMap.get(id) || 0;me}">${safeName}</span>
      let col2Label, col2Val, col3Label, col3Val, progressVal = 0;/span>
                  </div>
      if (isDayMode) { class="mini-meta">${this.escapeHtml(timeDisplay)}</div>
        col2Label = (m === this.time.todayStr) ? "今日" : "当日";
        col2Val = selectedTargetVal;
        col3Val = monthContextVal;d" onclick="openLog('${safeId}','${safeM}','${encodedName}')">
        progressVal = viewTotalDenominator > 0 ? ((monthContextVal / viewTotalDenominator) * 100).toFixed(1) : 0;
      } else {  <div class="stat-title-wrap">
        col2Label = "今日";lass="stat-emoji">${this.escapeHtml(emoji || '🔗')}</span>
        col2Val = realTodayVal;stat-title">${safeName}</span>
        col3Label = (m === this.time.dateKey) ? "本月" : "当月";
        col3Val = selectedTargetVal;t">${progressVal}%</span>
        progressVal = viewTotalDenominator > 0 ? ((selectedTargetVal / viewTotalDenominator) * 100).toFixed(1) : 0;
      }       <div class="stat-metrics">
                <div class="metric">
      let timeDisplay = stat.last_time || '暂无';</span>
      let timeIcon = '🕒';ass="metric-value">${stat.total_clicks || 0}</span>
      if (timeDisplay !== '暂无') {
        if (isDayMode) {ss="metric">
          timeDisplay = timeDisplay.split(' ')[1] || timeDisplay;
        } else {  <span class="metric-value metric-gold">${col2Val}</span>
          timeDisplay = timeDisplay.split(' ')[0].substring(5);
          timeIcon = '📅';="metric">
      ...
      const safeId = this.escapeAttr(id || '');
      const safeM = this.escapeAttr(m || '');e="width:${progressVal}%"></div></div>
      const safeName = this.escapeHtml(name || '');this.escapeHtml(timeDisplay)}</div>
      const encodedName = encodeURIComponent(name || '');
    };
        return `<div class="mini-card" onclick="openLog('${safeId}','${safeM}','${encodedName}')">
                  <div class="mini-top"> => buildCard(i.id, i.name, '', true)).join('');
                    <span class="mini-badge">${selectedTargetVal}</span>
                  </div>onfig.admin_pass,
                  <div class="mini-meta">${this.escapeHtml(timeDisplay)}</div>
                </div>`;fig.subtitle,
      }mg: this.dbSettings.img || this.env.img || "",
      contact_url: this.config.contact_url,
      return `<div class="stat-card" onclick="openLog('${safeId}','${safeM}','${encodedName}')">
              <div class="stat-top">this.env.push || "",
                <div class="stat-title-wrap">host || "",
                  <span class="stat-emoji">${this.escapeHtml(emoji || '🔗')}</span>
                </div>is.config.promo_enable,
                <span class="stat-pct">${progressVal}%</span>
              </div>his.config.promo_title,
              <div class="stat-metrics">,
                <div class="metric">rl,
                  <span class="metric-label">历史</span>
                </div>this.config.account_enable,
                <div class="metric">count_format,
                  <span class="metric-label">${col2Label}</span>
                </div>ngify(this.LINKS_DATA, null, 2),
                <div class="metric">RIENDS_DATA, null, 2)
                  <span class="metric-label">${col3Label}</span>
                </div>iew = '';
              </div>notice && this.config.notice.trim() !== '') {
              <div class="progress"><div style="width:${progressVal}%"></div></div><span>❤️</span><strong>公告预览</strong></div><div class="notice-preview">${this.config.notice}</div></div>`;
              <div class="stat-foot">${timeIcon} ${this.escapeHtml(timeDisplay)}</div>
    };nst promoPreview = String(this.config.promo_enable) === '1'
      ? `<div class="panel notice-panel"><div class="panel-head"><span>📣</span><strong>推广卡预览（首页中部）</strong></div><div class="promo-preview-box"><div class="promo-preview-badge">${this.escapeHtml(this.config.promo_badge || '推广支持')}</div><div class="promo-preview-main"><div class="promo-preview-title">${this.escapeHtml(this.config.promo_title || '推广支持')}</div><div class="promo-preview-desc">${this.renderRichContent(this.config.promo_desc || '', this.config.promo_format)}</div></div></div></div>`
    const linkHtml = safeLinks.map(i => buildCard(i.id, i.name, i.emoji, false)).join('');
    const accountPreview =
    const sysSettings = {.account_enable || '0') === '1' && String(this.config.account_content || '').trim())
      admin_pass: this.config.admin_pass,
      title: this.config.title,.renderRichContent(this.config.account_content || '', this.config.account_format || 'markdown');
      subtitle: this.config.subtitle,ce(/<div class="ad-badge">[\s\S]*?<\/div>/i, '');
      contact_url: this.config.contact_url,e-panel">
      mail: this.config.mail,el-head"><span>🧾</span><strong>账号推广卡预览</strong></div>
      push: this.dbSettings.push || this.env.push || "",o-preview-box">
      host: this.dbSettings.host || this.env.host || "",preview-badge">账号购买</div>
      notice: this.config.notice,-preview-main">
    ...
    const promoPreview = String(this.config.promo_enable) === '1'
      ? `<div class="panel notice-panel"><div class="panel-head"><span>📣</span><strong>推广卡预览（首页中部）</strong></div><div class="promo-preview-box"><div class="promo-preview-badge">${this.escapeHtml(this.config.promo_badge || '推广支持')}</div><div class="promo-preview-main"><div class="promo-preview-title">${this.escapeHtml(this.config.promo_title || '推广支持')}</div><div class="promo-preview-desc">${this.renderRichContent(this.config.promo_desc || '', this.config.promo_format)}</div></div></div></div>`
      : '';hadow-soft:0 4px 14px rgba(2,6,23,0.12);
      }
    const accountPreview =
        ...
        --bd-strong:rgba(15,23,42,0.12);
            return `<div class="panel notice-panel">
              ...
              <div class="promo-preview-box account-promo-preview-box">
                ...
                <div class="promo-preview-main">.06);
                  <div class="promo-preview-desc account-preview-desc">${rendered}</div>
                </div>der-box}
              </div>havior:smooth}
            </div>`;
    ...
    return `<!DOCTYPE html><html lang="zh-CN"><head>${this.render_Head(this.config.title)}<style>
      :root{in:0;
        ...
        --bg-card-soft:rgba(15,23,42,0.58); margin:0 auto; }
        ...
        --blue:#38bdf8;
        --violet:#8b5cf6;x;
        --gold:#fbbf24;d var(--bd);
        --green:#34d399;-bg-card-soft);
        --red:#f87171;t);
      ...
      .light-theme{ter;
        ...
        --txt:#0f172a;
        --txt-sub:#475569;g-card);
        --txt-soft:#64748b;r(--bd);
        --shadow:0 10px 24px rgba(15,23,42,0.08);
        --shadow-soft:0 4px 12px rgba(15,23,42,0.06);
      } box-shadow:var(--shadow);
      *{box-sizing:border-box}
      html{scroll-behavior:smooth}
      body{d-template-columns:1.4fr 1fr;
        color:var(--txt);
        ${this.getBgShellStyle()}
        padding:24px;
        display:block;
        margin:0;:clamp(1.9rem,4vw,2.8rem);
      } line-height:1.08;
      .admin-shell{ width:min(1320px,100%); margin:0 auto; }
        margin:0 0 10px;
      .theme-toggle{900;
      ...
      .hero-title{
        ...
        margin:0 0 10px;lumns:repeat(3,1fr);
        font-weight:900;
      }
      .hero-sub{d{
        color:var(--txt-soft);ev);
        line-height:1.7; var(--bd);
        font-size:.98rem;x;
        max-width:760px;
      } min-height:92px;
      .hero-tags{lex;
        margin-top:18px;olumn;
        display:flex;nt:space-between;
        gap:10px;
        flex-wrap:wrap;
      } color:var(--txt-sub);
      .pill{-size:.82rem;
        ...
        gap:12px;
      .top-actions{rap;
        display:flex;nt:flex-end;
        flex-direction:column;
        justify-content:space-between;
        gap:16px;{
      } appearance:none;
      .quick-stats{;
        display:grid;on:none;
      ...
      .quick-card{t:800;
        ...
        border-radius:18px;;
        padding:16px;enter;
        min-height:92px;center;
        display:flex;
        flex-direction:column;w-soft);
        justify-content:space-between;
      }action-btn:hover{ transform:translateY(-1px); }
      .quick-label{ry{ background:linear-gradient(135deg,#3b82f6,#8b5cf6); color:#fff; }
        color:var(--txt-sub);d:var(--bg-elev); color:var(--txt); border:1px solid var(--bd); }
        font-size:.82rem;kground:rgba(248,113,113,.14); color:var(--red); border:1px solid rgba(248,113,113,.18); }
        letter-spacing:.03em;
      } display:flex;
      .quick-value{:center;
        font-size:1.76rem;ace-between;
        font-weight:900;
        line-height:1;6px;
      } background:var(--bg-card-soft);
        border:1px solid var(--bd);
      .action-row{ius:20px;
        display:flex;er:blur(12px);
        gap:12px;w:var(--shadow-soft);
        flex-wrap:wrap;0px;
      ...
      .action-btn{ex;
        ...
        cursor:pointer;
        padding:11px 15px;
        border-radius:14px;e;
        font-weight:800;;
        font-size:.92rem;bg-elev);
        transition:.18s ease;--bd);
        display:inline-flex;
        align-items:center;
        justify-content:center;
        gap:8px;ght:800;
      ...
      .tbtn{or:pointer;
        text-decoration:none;
        background:var(--bg-elev);id-template-columns:1fr; gap:20px; }
        border:1px solid var(--bd);
        padding:10px 14px;g-card);
        border-radius:12px;r(--bd);
        font-size:.9rem;px;
        font-weight:800;blur(14px);
        transition:.16s ease;ow);
      } padding:20px;
      ...
      .date-chip strong{
        ...
        position:absolute;
        inset:0;grid;
        opacity:0;ate-columns:repeat(auto-fill,minmax(280px,1fr));
        cursor:pointer;
      }
      .stat-card{
      .grid-main{ display:grid; grid-template-columns:1fr; gap:20px; }
      .panel{r:1px solid var(--bd);
        background:var(--bg-card);(180deg,var(--bg-elev),rgba(255,255,255,0.02));
        border:1px solid var(--bd);
        border-radius:24px;
        backdrop-filter:blur(14px);
        box-shadow:var(--shadow);
        padding:20px;ility:auto;
      } contain-intrinsic-size:166px;
      .panel-header{
        display:flex;r{
        align-items:center;Y(-2px);
        justify-content:space-between;24);
        gap:12px;w:0 10px 24px rgba(2,6,23,0.12);
        margin-bottom:18px;
        flex-wrap:wrap;
      } display:flex;
      .panel-title{:flex-start;
        margin:0;ontent:space-between;
        font-size:1rem;
        letter-spacing:.04em;
      ...
      .stat-card{ontent:center;
        border-radius:20px;,255,255,.06);
        border:1px solid var(--bd);
      ...
      .stat-card:hover{llipsis;
        transform:translateY(-2px);
        border-color:rgba(56,189,248,.24);
        box-shadow:0 10px 24px rgba(2,6,23,0.12);
      } padding:6px 10px;
      .stat-top{adius:999px;
        display:flex;ba(56,189,248,.12);
        align-items:flex-start;
        justify-content:space-between;
        gap:12px;:.82rem;
        margin-bottom:16px;ba(56,189,248,.16);
      }
        display:flex;
        align-items:center;
        gap:12px;late-columns:repeat(3,1fr);
        min-width:0;
        flex:1;bottom:14px;
      }
      .stat-emoji{
        width:42px;rgba(255,255,255,.02);
        height:42px;olid var(--bd);
        border-radius:14px;
        display:flex;10px;
        align-items:center;
        justify-content:center;
      ...
      .stat-pct{ld{ color:var(--gold); }
        flex-shrink:0;lor:var(--blue); }
        border-radius:999px;
        background:rgba(56,189,248,.12);
        color:#7dd3fc;999px;
        font-weight:900;255,255,255,.06);
        font-size:.82rem;
      ...
      .stat-metrics{
        display:grid;:999px;
        grid-template-columns:repeat(3,1fr);bf24,#38bdf8,#8b5cf6);
        gap:10px;
        margin-bottom:14px;
      } color:var(--txt-sub);
      .metric{ize:.82rem;
      ...
      .metric-label{e-columns:repeat(auto-fill,minmax(180px,1fr));
        display:block;
        font-size:.74rem;
        color:var(--txt-sub);
        margin-bottom:5px;gradient(180deg,var(--bg-elev),rgba(255,255,255,0.02));
        font-weight:700; var(--bd);
      } border-radius:18px;
      .metric-value{;
        font-size:1.05rem;
        font-weight:900;ease;
        color:var(--txt);y:auto;
      } contain-intrinsic-size:90px;
      .metric-gold{ color:var(--gold); }
      .metric-blue{ color:var(--blue); }
        transform:translateY(-2px);
      .progress{olor:rgba(56,189,248,.24);
        height:8px;
        border-radius:999px;
        background:rgba(255,255,255,.06);
        overflow:hidden;er;
      ...
      .stat-foot{ce:nowrap;
        color:var(--txt-sub);
        font-size:.82rem;ipsis;
        font-family:monospace;
        text-align:right;
      }mini-badge{
        flex-shrink:0;
      .mini-grid{dius:999px;
        display:grid;px;
        grid-template-columns:repeat(auto-fill,minmax(180px,1fr));
        gap:12px;x solid rgba(56,189,248,.16);
      } color:#7dd3fc;
      .mini-card{ht:900;
        background:linear-gradient(180deg,var(--bg-elev),rgba(255,255,255,0.02));
        border:1px solid var(--bd);
        ...
        content-visibility:auto;
        contain-intrinsic-size:90px;
      .mini-card:hover{dding:18px 20px; }
        transform:translateY(-2px);
        border-color:rgba(56,189,248,.24);
      } align-items:center;
      .mini-top{;
      ...
      .mini-badge{px;
        flex-shrink:0;18px;
        border-radius:999px;adient(135deg, rgba(255,255,255,0.08), rgba(59,130,246,0.06));
        padding:4px 8px; var(--bd);
        background:rgba(56,189,248,.12);
        border:1px solid rgba(56,189,248,.16);
        color:#7dd3fc;x;
        font-weight:900;x;
      ...
      .mini-meta{d:rgba(255,255,255,0.08);
        text-align:right;var(--bd);
        color:var(--txt-sub);
        font-family:monospace;
        font-size:.78rem;
      }promo-preview-main{ flex:1; min-width:0; }
      ...
      .notice-preview{esc p:last-child{ margin-bottom:0; }
        color:var(--txt-soft);margin:4px 0 0 18px; padding:0; }
        line-height:1.8;c li{ margin:4px 0; }
        font-size:.94rem; code{
      } padding:2px 6px;
        border-radius:8px;
      .promo-preview-box{55,255,255,.06);
        display:flex;lid var(--bd);
        gap:16px;ly:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace;
        align-items:flex-start;
        padding:14px;desc a{ color:#93c5fd; }
        background:linear-gradient(135deg, rgba(255,255,255,0.08), rgba(59,130,246,0.06));
        border:1px solid var(--bd);135deg, rgba(255,255,255,0.08), rgba(16,185,129,0.08));
      }
      .promo-preview-badge{e{
        min-width:130px;
        padding:10px 14px;gradient(135deg, rgba(16,185,129,0.24), rgba(59,130,246,0.14));
        border-radius:999px;a(167,243,208,0.18);
        text-align:center;
      ...
      .promo-preview-main{ flex:1; min-width:0; }
      .promo-preview-title{ h2,
        font-weight:900;esc h3{ margin:0 0 8px; line-height:1.35; }
        margin-bottom:8px;c p{ margin:0 0 8px; }
      }account-preview-desc p:last-child{ margin-bottom:0; }
      .promo-preview-desc{c ul{ margin:4px 0 0 18px; padding:0; }
        color:var(--txt-soft);{ margin:4px 0; }
        line-height:1.75;sc code{
        font-size:.93rem;
      } border-radius:8px;
      .promo-preview-desc p{ margin:0 0 8px; }
      ...
      .promo-preview-desc code{-badge{ display:none !important; }
        padding:2px 6px;esc .ad-btn{
        border-radius:8px;x;
        background:rgba(255,255,255,.06);
        border:1px solid var(--bd);
      ...
      .account-preview-badge{(255,255,255,.12);
        color:#d1fae5;
        border:1px solid rgba(167,243,208,0.18);
      } position:fixed;
      .account-preview-desc{
        color:var(--txt-soft);,.44);
        line-height:1.75;
        font-size:.93rem;
      } pointer-events:none;
      ...
      .account-preview-desc .ad-btn{
        display:inline-flex;card);
        align-items:center;id var(--bd);
        justify-content:center;px);
        margin-top:8px;x 0 24px rgba(2,6,23,.16);
        padding:9px 12px;
        border-radius:10px;mn;
        text-decoration:none;
        color:#fff; right:0; }
        font-weight:800;
        background:linear-gradient(135deg,#3b82f6,#8b5cf6);
        ...
        align-items:center;
      .mask{ify-content:space-between;
        position:fixed;
        inset:0;
      ...
      .mask.show{x;
        ...
        cursor:pointer;
      .drawer{ound:var(--bg-elev);
        position:fixed;);
        top:0;:1px solid var(--bd);
        right:-440px;rem;
        width:400px;
        max-width:100vw;
        height:100vh;
      ...
      .drawer-title{
        margin:0;ttom:8px;
        font-size:1.05rem;
        font-weight:900;#7dd3fc; font-weight:900; }
      }log-time{ color:var(--txt); font-size:.86rem; }
      .icon-btn{
        width:36px;x;
        height:36px;ent:space-between;
        border:none;
        border-radius:12px;
        cursor:pointer;ospace;
        background:var(--bg-elev);
        color:var(--txt);
        border:1px solid var(--bd);
      }fs-modal{
      ...
      .log-item{8px auto;
        padding:14px;r(--bg-card);
        border-radius:16px;r(--bd);
        border:1px solid var(--bd);
        background:rgba(255,255,255,.02);
      } overflow:hidden;
      .log-row{
        display:flex;
        align-items:center;
        justify-content:space-between;bd);
        gap:10px;lex;
        margin-bottom:8px;;
      } justify-content:space-between;
      ...
      .settings-wrap{
        ...
        margin:18px auto;
        background:var(--bg-card);
        border:1px solid var(--bd);t(2,1fr);
        border-radius:28px;
        box-shadow:var(--shadow);
        overflow:hidden;:1 / -1; }
      .settings-head{
        padding:20px 22px;5,255,255,.02);
        border-bottom:1px solid var(--bd);
        display:flex;:18px;
        align-items:center;
        justify-content:space-between;
      ...
      .settings-sub{1.6;
        color:var(--txt-sub);
        font-size:.9rem;d textarea,.field select{
        margin-top:6px;
      } border-radius:14px;
      .settings-actions{ var(--bd-strong);
        display:flex;ba(2,6,23,.18);
        gap:10px;(--txt);
        flex-wrap:wrap;
      } font-size:.95rem;
      .settings-body{ padding:22px; }no-Regular,Menlo,Monaco,Consolas,'Liberation Mono','Courier New',monospace;
      .settings-grid{
        display:grid;6s ease;
        grid-template-columns:repeat(2,1fr);
        gap:16px;e .field input,
      }light-theme .field textarea,
      .full{ grid-column:1 / -1; }
        background:#fff;
      .field{
        background:rgba(255,255,255,.02);focus,.field select:focus{
        border:1px solid var(--bd);8,.42);
        border-radius:18px;x rgba(56,189,248,.10);
        padding:16px;
      }field textarea{
      .field label{130px;
        display:block;l;
        ...
        color:var(--txt-sub);
        margin-bottom:10px;
        font-weight:800;de{ min-height:250px; }
      .field small{
        display:block;
        color:var(--txt-sub);
        font-size:.78rem;
      ...
      .field input,.field textarea,.field select{
        ...
        color:var(--txt);
        padding:14px;rem;
        font-size:.95rem;bg-elev);
        font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,'Liberation Mono','Courier New',monospace;
        outline:none;lid var(--bd);
        transition:.16s ease;
      }
      .light-theme .field input,d:rgba(56,189,248,.12); border-color:rgba(56,189,248,.22); color:#7dd3fc; }
      ...
      .field textarea{ive;
        min-height:130px;
        resize:vertical;
        line-height:1.55;ock;
        white-space:pre;
      }switch input{ display:none; }
      .field textarea.code{ min-height:250px; }
        position:absolute;
      .field-tools{
        display:flex;ba(255,255,255,.10);
        gap:8px;px solid var(--bd);
        flex-wrap:wrap;99px;
      ...
      .mini-btn{fore{
        ...
        background:var(--bg-elev);
        color:var(--txt);ase;
        border:1px solid var(--bd);,0,0,.18);
        transition:.16s ease;
      }switch input:checked + .slider{
      .mini-btn:hover{ background:rgba(56,189,248,.12); border-color:rgba(56,189,248,.22); color:#7dd3fc; }
        border-color:rgba(56,189,248,.28);
      .switch-row{
        display:flex;hecked + .slider:before{
        align-items:center;X(22px);
        gap:12px;
      }media (max-width: 1024px){
      .switch{r{ grid-template-columns:1fr; }
        position:relative;
        width:52px;idth: 768px){
        height:30px;g:16px; }
        display:inline-block;mplate-columns:1fr; }
      } .action-row{ justify-content:flex-start; }
      .switch input{ display:none; }
      .slider{-grid{ grid-template-columns:1fr; }
        position:absolute;id-template-columns:1fr; }
        inset:0; width:100%; right:-100%; }
        background:rgba(255,255,255,.10); 10px); margin:5px auto; border-radius:20px; }
        border:1px solid var(--bd);dy{ padding:16px; }
        border-radius:999px;flex-direction:column; }
        transition:.18s ease; min-width:auto; width:fit-content; }
      }
      .slider:before{
        content:'';
        position:absolute;me-toggle" onclick="toggleAdminTheme()" title="切换主题">☀️</button>
        height:22px;admin-shell">
        left:3px;n class="topbar">
        top:3px;>
        background:#fff;"hero-title">📊 数据看板</h1>
        border-radius:50%;hero-sub">
        transition:.18s ease;g style="color:var(--txt)">${this.escapeHtml(m)}</strong>。你可以在这里查看精选资源与友链的点击情况、快速预览公告与推广卡内容，并直接进入系统配置面板修改站点设置。
        box-shadow:0 2px 8px rgba(0,0,0,.18);
      }       <div class="hero-tags">
      .switch input:checked + .slider{路径：${this.escapeHtml(this.ADMIN_PATH)}</span>
        background:rgba(56,189,248,.26);：${this.escapeHtml(this.time.fullStr)}</span>
        border-color:rgba(56,189,248,.28);：${historyTotal}</span>
      }       </div>
      .switch input:checked + .slider:before{
      }     <div class="top-actions">
              <div class="quick-stats">
      @media (max-width: 1024px){-card">
        .topbar{ grid-template-columns:1fr; }目</div>
      }           <div class="quick-value">${safeLinks.length}</div>
      @media (max-width: 768px){
        body{ padding:16px; }uick-card">
        .quick-stats{ grid-template-columns:1fr; }div>
        .action-row{ justify-content:flex-start; }color:var(--blue)">${monthTotalClicks}</div>
        .toolbar{ padding:14px; }
        .stats-grid{ grid-template-columns:1fr; }
        .settings-grid{ grid-template-columns:1fr; }>
        .drawer{ width:100%; right:-100%; }${Array.from(statsMap.values()).filter(c => c.total_clicks > 0).length}</div>
        .settings-wrap{ width:calc(100% - 10px); margin:5px auto; border-radius:20px; }
        .settings-head,.settings-body{ padding:16px; }
        .promo-preview-box{ flex-direction:column; }
        .promo-preview-badge{ min-width:auto; width:fit-content; }</a>
      }         <button type="button" class="action-btn action-primary" onclick="openSettings()">⚙️ 系统设置</button>
      </style></head>ef="${this.ADMIN_PATH}/logout" class="action-btn action-danger">登出</a>
      <body>  </div>
        <button class="theme-toggle" onclick="toggleAdminTheme()" title="切换主题">☀️</button>
          </section>
          <section class="topbar">>
            <div>class="toolbar-left">
              <h1 class="hero-title">📊 数据看板</h1>vMonthStr}" class="tbtn" title="上个月">⏪ 上月</a>
              <div class="hero-sub">PATH}?m=${prevDay}" class="tbtn">◀ 上一项</a>
                当前查看维度：<strong style="color:var(--txt)">${this.escapeHtml(m)}</strong>。你可以在这里查看精选资源与友链的点击情况、快速预览公告与推广卡内容，并直接进入系统配置面板修改站点设置。
              </div>n>📅</span>
              <div class="hero-tags">Html(m)}</strong>
                <span class="pill">🧭 路径：${this.escapeHtml(this.ADMIN_PATH)}</span>nchange="if(this.value) location.href='${this.ADMIN_PATH}?m='+this.value">
                <span class="pill">🕒 时间：${this.escapeHtml(this.time.fullStr)}</span>
                <span class="pill">📦 历史总计：${historyTotal}</span>tn">下一项 ▶</a>
              </div>f="${this.ADMIN_PATH}?m=${nextMonthStr}" class="tbtn" title="下个月">下月 ⏩</a>
            </div>
            <div class="toolbar-right">
            <div class="top-actions">ATH}?m=${this.time.todayStr}" class="tbtn ${m===this.time.todayStr?'active':''}">今日</a>
              <div class="quick-stats">H}?m=${this.time.dateKey}" class="tbtn ${m===this.time.dateKey?'active':''}">本月</a>
                ...
                </div>class="panel-title">💎 精选数据</h3>
              </div>iv class="panel-sub">点击任意卡片可查看访问记录详情</div>
              <div class="action-row">
                <a href="/" class="action-btn action-soft">🏠 返回主页</a>
                <button type="button" class="action-btn action-primary" onclick="openSettings()">⚙️ 系统设置</button>
                <a href="${this.ADMIN_PATH}/logout" class="action-btn action-danger">登出</a>
            </div>on class="panel">
          </section>lass="panel-header">
                <div>
          <section class="toolbar">title">🔗 友链数据</h3>
            <div class="toolbar-left">b">简要查看友链表现</div>
              <a href="${this.ADMIN_PATH}?m=${prevMonthStr}" class="tbtn" title="上个月">⏪ 上月</a>
              <a href="${this.ADMIN_PATH}?m=${prevDay}" class="tbtn">◀ 上一项</a>
              <div class="date-chip" title="点击切换日期">div>
                <span>📅</span>
                <strong>${this.escapeHtml(m)}</strong>
                <input type="date" value="${isDayMode ? this.escapeAttr(m) : ''}" onchange="if(this.value) location.href='${this.ADMIN_PATH}?m='+this.value">
              <a href="${this.ADMIN_PATH}?m=${nextDay}" class="tbtn">下一项 ▶</a>
          ...
          </section>"log-list" id="dl"></ul>
        </aside>
            ${noticeHtmlPreview}="set-fs">
            ${promoPreview}ngs-wrap">
            ${accountPreview}ngs-head">
              <div>
            <section class="panel">-title">⚙️ 系统全局配置</h3>
              <div class="panel-header">">支持 JSON 自动格式化、示例模板填充、推广卡 HTML / Markdown 内容配置、账号推广卡配置。</div>
                <div>
                  <h3 class="panel-title">💎 精选数据</h3>
                  <div class="panel-sub">点击任意卡片可查看访问记录详情</div>-soft" onclick="cls()">取消 (Esc)</button>
                </div>n type="button" class="action-btn action-primary" onclick="saveSettings(this)">💾 保存并生效</button>
              </div>
              <div class="stats-grid">${linkHtml}</div>
            <div class="settings-body">
            <section class="panel">grid">
              <div class="panel-header">
                <div>bel>后台登录密码</label>
                  <h3 class="panel-title">🔗 友链数据</h3>holder="默认: 123456">
                  <div class="panel-sub">简要查看友链表现</div>
              </div> class="field">
              <div class="mini-grid">${friendHtml}</div>
            </section>ut type="text" id="s_title">
          </div></div>
                <div class="field">
        <div class="mask" id="mask" onclick="cls()"></div>
        ...
        <aside class="drawer" id="dr">
            <h3 class="drawer-title" id="dt">点击记录</h3>
            <button type="button" class="icon-btn" onclick="cls()">×</button>
          </div>  <input type="text" id="s_tg" placeholder="例如 Telegram / 工单地址">
          <ul class="log-list" id="dl"></ul>
                <div class="field full">
        <div class="fs-modal" id="set-fs">持 Base64）</label>
          <div class="settings-wrap">id="s_img" placeholder="留空使用默认高清壁纸">
            <div class="settings-head">兼容的图片地址。如果图床开启防盗链，部分浏览器会回退到默认背景。</small>
              <div>iv>
                <div class="settings-sub">支持 JSON 自动格式化、示例模板填充、推广卡 HTML / Markdown 内容配置、账号推广卡配置。</div>
              </div>abel>联系邮箱</label>
              <div class="settings-actions">ail" placeholder="留空则不显示底部邮箱按钮">
                <button type="button" class="action-btn action-soft" onclick="cls()">取消 (Esc)</button>
              </div> class="field">
            </div><label>留言板推送 Webhook</label>
                  <input type="text" id="s_push" placeholder="留空则不显示留言按钮">
            <div class="settings-body">
                <div class="field">ull">
                  <label>后台登录密码</label>，清空则隐藏）</label>
                  <input type="text" id="s_pass" placeholder="默认: 123456">
                </div>
                <div class="field">ull">
                  <label>网站主标题</label>中部）</label>
                  <input type="text" id="s_title">argin-bottom:12px;">
                </div>abel class="switch">
                      <input type="checkbox" id="s_promo_enable">
                <div class="field">slider"></span>
                  <label>网站副标题</label>
                  <input type="text" id="s_sub">-sub);font-weight:700;">启用首页推广卡</span>
                </div>v>
                  <div class="settings-grid">
                <div class="field">ld">
                  <label>客服支持链接</label>abel>
                  <input type="text" id="s_tg" placeholder="例如 Telegram / 工单地址">托管 CF">
                </div>div>
                    <div class="field">
                <div class="field full">l>
                  <label>背景图 URL（多图用逗号隔开，支持 Base64）</label> placeholder="如：本站域名服务由 ... 提供支持">
                  <input type="text" id="s_img" placeholder="留空使用默认高清壁纸">
                  <small>建议使用可公开访问、跨浏览器兼容的图片地址。如果图床开启防盗链，部分浏览器会回退到默认背景。</small>
                </div><label>推广卡跳转链接</label>
                      <input type="text" id="s_promo_url" placeholder="https://...">
                <div class="field">
                  <label>联系邮箱</label>">
                  <input type="text" id="s_mail" placeholder="留空则不显示底部邮箱按钮">
                </div><select id="s_promo_format">
                        <option value="markdown">Markdown</option>
                <div class="field">ue="html">HTML</option>
                  <label>留言板推送 Webhook</label>
                  <input type="text" id="s_push" placeholder="留空则不显示留言按钮">
                </div>v>
                  <div style="margin-top:16px;">
                <div class="field full">y:block;margin-bottom:10px;font-size:.88rem;color:var(--txt-sub);font-weight:800;">推广卡描述内容</label>
                  <label>温馨提示公告（支持 HTML，清空则隐藏）</label>"min-height:150px"></textarea>
                  <textarea id="s_notice"></textarea>n 支持标题、加粗、斜体、列表、链接、行内代码等基础语法。</small>
                </div>v>
                </div>
                  <label>推广卡开关 / 配置（首页中部）</label>
                  <div class="switch-row" style="margin-bottom:12px;">
                    <label class="switch">style="margin-bottom:12px;">
                      <input type="checkbox" id="s_promo_enable">
                      <span class="slider"></span>_account_enable">
                    </label>class="slider"></span>
                    <span style="color:var(--txt-sub);font-weight:700;">启用首页推广卡</span>
                  </div>n style="color:var(--txt-sub);font-weight:700;">启用首页账号推广卡</span>
                  <div class="settings-grid">
                      <label>推广卡徽标文字</label>>
                      <input type="text" id="s_promo_badge" placeholder="如：免费域名可托管 CF">
                    </div>el>内容格式</label>
                    <div class="field">ount_format">
                      <label>推广卡标题</label>kdown">Markdown</option>
                    ...
                    <div class="field">-tools" style="margin-bottom:0;">
                      <label>推广卡内容格式</label>" class="mini-btn" onclick="fillAccountMarkdownExample()">填入 Markdown 示例</button>
                      <select id="s_promo_format">s="mini-btn" onclick="fillAccountHtmlExample()">填入 HTML 示例</button>
                        <option value="markdown">Markdown</option>
                        <option value="html">HTML</option>HTML。</small>
                      </select>
                    </div>
                  <div style="margin-top:16px;">
                    <label style="display:block;margin-bottom:10px;font-size:.88rem;color:var(--txt-sub);font-weight:800;">推广卡描述内容</label>
                    <textarea id="s_promo_desc" style="min-height:150px"></textarea>220px" placeholder="可直接粘贴 Markdown 或 HTML 片段"></textarea>
                    <small>支持 Markdown 或 HTML。Markdown 支持标题、加粗、斜体、列表、链接、行内代码等基础语法。</small>
                  </div>
                </div>
                <div class="field full">
                  <label>账号推广卡开关 / 配置（首页中部）</label>
                  <div class="switch-row" style="margin-bottom:12px;">动使用当前访问域名">
                    <label class="switch">
                      <span class="slider"></span>
                    </label>精选资源 LINKS（JSON 格式）</label>
                    <span style="color:var(--txt-sub);font-weight:700;">启用首页账号推广卡</span>
                  </div>ton type="button" class="mini-btn" onclick="formatJsonField('s_links')">自动格式化</button>
                    <button type="button" class="mini-btn" onclick="fillLinksExample()">填入示例模板</button>
                  <div class="settings-grid">
                    <div class="field">" class="code"></textarea>
                      <label>内容格式</label>
                        <option value="markdown">Markdown</option>
                        <option value="html">HTML</option>
                      </select>ield-tools">
                    </div>n type="button" class="mini-btn" onclick="formatJsonField('s_friends')">自动格式化</button>
                    <button type="button" class="mini-btn" onclick="fillFriendsExample()">填入示例模板</button>
                    <div class="field">
                      <label>快捷填充</label>" class="code"></textarea>
                      <div class="field-tools" style="margin-bottom:0;">
                        <button type="button" class="mini-btn" onclick="fillAccountMarkdownExample()">填入 Markdown 示例</button>
                        <button type="button" class="mini-btn" onclick="fillAccountHtmlExample()">填入 HTML 示例</button>
                      </div>
                ...
                </div>EXAMPLE = [
            {
                ...
                </div>rl: "https://www.google.com.hk",
              emoji: "🔎",
                <div class="field full">
                  <label>💎 精选资源 LINKS（JSON 格式）</label>
                  <div class="field-tools">
        ...
        ${this.render_BgRuntimeScript()}
          ...
          const ADMIN_PATH = '${this.ADMIN_PATH}';tagram / X\\n\\n' +
          const SYS_SET = ${this.safeScriptJson(sysSettings)};
            '[👉 立即进入购买通道](https://tgsss.com/9EB6941B)';
            {st ACCOUNT_HTML_EXAMPLE =
              id: "google",gle / Apple 外区 ID / Telegram / Instagram / X</strong></p>\\n' +
              name: "Google 搜索",自动发货</p>\\n' +
              url: "https://www.google.com",EB6941B" target="_blank" rel="noopener noreferrer">👉 立即进入购买通道</a></p>';
              emoji: "🔎",inTheme() {
              note: "全球常用搜索引擎",t.querySelector('.theme-toggle');
              tag: "推荐"eturn;
            {f (localStorage.getItem('admin_theme') === 'light') {
              id: "github",.classList.add('light-theme');
              name: "GitHub", = '🌙';
          ...
          const FRIENDS_EXAMPLE = [() {
            {onst btn = document.querySelector('.theme-toggle');
              id: "friend_1",ssList.toggle('light-theme');
              name: "示例友链站点",ocument.body.classList.contains('light-theme');
              url: "https://example.com"theme', isLight ? 'light' : 'dark');
            }, (btn) btn.textContent = isLight ? '🌙' : '☀️';
            {
              name: "另一个合作伙伴",og(id, m, n) {
              url: "https://example.org"ntById('dr');
            }onst mask = document.getElementById('mask');
          ];const l = document.getElementById('dl');
          const ACCOUNT_MD_EXAMPLE =;
            ...
            '<p><a href="https://tgsss.com/9EB6941B" target="_blank" rel="noopener noreferrer">👉 立即进入购买通道</a></p>';
              const r = await fetch(
          function initAdminTheme() {gs?id=' + encodeURIComponent(id) + '&m=' + encodeURIComponent(m)
            const btn = document.querySelector('.theme-toggle');
            if (!btn) return;ait r.json();
            if (localStorage.getItem('admin_theme') === 'light') {
              document.body.classList.add('light-theme');xt-align:center;opacity:.6;color:var(--txt-sub)">该时段无记录</li>';
              btn.textContent = '🌙';
            } else {
            } let html = '';
          }   for (let i = 0; i < data.length; i++) {
          initAdminTheme();ata[i];
                html += '<li class="log-item">'
          function toggleAdminTheme() {w">'
            const btn = document.querySelector('.theme-toggle');pan>'
            document.body.classList.toggle('light-theme');ime || '') + '</span>'
            const isLight = document.body.classList.contains('light-theme');
            localStorage.setItem('admin_theme', isLight ? 'light' : 'dark');
            if (btn) btn.textContent = isLight ? '🌙' : '☀️';'</span>'
          ...
          async function openLog(id, m, n) {
            ...
            l.innerHTML = '<li style="padding:20px;text-align:center;color:var(--txt-sub)">加载中...</li>';
            document.getElementById('s_sub').value = SYS_SET.subtitle || '';
            try {ent.getElementById('s_img').value = SYS_SET.img || '';
              const r = await fetch('s_tg').value = SYS_SET.contact_url || '';
                ADMIN_PATH + '/api/logs?id=' + encodeURIComponent(id) + '&m=' + encodeURIComponent(m)
              );ment.getElementById('s_push').value = SYS_SET.push || '';
              const data = await r.json();t').value = SYS_SET.host || '';
            document.getElementById('s_notice').value = SYS_SET.notice || '';
                l.innerHTML = '<li style="padding:20px;text-align:center;opacity:.6;color:var(--txt-sub)">该时段无记录</li>';
                return;tElementById('s_promo_badge').value = SYS_SET.promo_badge || '';
              }ument.getElementById('s_promo_title').value = SYS_SET.promo_title || '';
            document.getElementById('s_promo_desc').value = SYS_SET.promo_desc || '';
              let html = '';entById('s_promo_url').value = SYS_SET.promo_url || '';
              for (let i = 0; i < data.length; i++) {.value = SYS_SET.promo_format || 'markdown';
                html += '<li class="log-item">'enable').checked = String(SYS_SET.account_enable || '0') === '1';
                  + '<div class="log-row">'unt_format').value = SYS_SET.account_format || 'markdown';
                  + '<span class="log-index">#' + (i + 1) + '</span>'SET.account_content || '';
                  + '</div>'entById('s_links').value = SYS_SET.links || '[]';
                  + '<div class="log-meta">'ds').value = SYS_SET.friends || '[]';
                  + '<span>' + (((x.user_agent || '').slice(0, 46)) || 'unknown') + '</span>'
                  + '</div>'entById('mask').classList.add('show');
                  + '</li>';yle.overflow = 'hidden';
              }
            } catch (e) {JsonField(id) {
              l.innerHTML = '<li style="padding:20px;text-align:center;color:#f87171">加载失败</li>';
              console.error(e);
          ...
          function openSettings() {
            ...
            document.getElementById('s_tg').value = SYS_SET.contact_url || '';
            document.getElementById('s_mail').value = SYS_SET.mail || '';
            document.getElementById('s_push').value = SYS_SET.push || '';)) return;
            document.getElementById('s_host').value = SYS_SET.host || '';
            document.getElementById('s_notice').value = SYS_SET.notice || '';
            document.getElementById('s_promo_enable').checked = String(SYS_SET.promo_enable || '0') === '1';
            document.getElementById('s_promo_badge').value = SYS_SET.promo_badge || '';
          ...
          function fillAccountHtmlExample() {
            document.getElementById('set-fs').classList.add('open');
            document.getElementById('mask').classList.add('show'); 示例覆盖吗？')) return;
            document.body.style.overflow = 'hidden';t').value = 'html';
          } el.value = ACCOUNT_HTML_EXAMPLE;
          }
            const el = document.getElementById(id);
            try {
              const parsed = JSON.parse(el.value);s_links').value);
              el.value = JSON.stringify(parsed, null, 2);ds').value);
              alert('✅ 已自动格式化');
            } catch (e) {SON 格式解析错误！请检查是否有遗漏的逗号、引号或括号。');
              alert('⚠️ JSON 格式有误，无法格式化');
            }
            const data = {
          function fillLinksExample() {lementById('s_pass').value,
            const el = document.getElementById('s_links');lue,
            if (el.value.trim() && !confirm('当前 LINKS 内容不为空，确定要用示例模板覆盖吗？')) return;
            el.value = JSON.stringify(LINKS_EXAMPLE, null, 2);
          }   contact_url: document.getElementById('s_tg').value,
              mail: document.getElementById('s_mail').value,
          function fillFriendsExample() {Id('s_push').value,
            const el = document.getElementById('s_friends');
            if (el.value.trim() && !confirm('当前 FRIENDS 内容不为空，确定要用示例模板覆盖吗？')) return;
          }   promo_enable: document.getElementById('s_promo_enable').checked ? '1' : '0',
              promo_badge: document.getElementById('s_promo_badge').value,
          function fillAccountMarkdownExample() {d('s_promo_title').value,
            const el = document.getElementById('s_account_content');lue,
            if (el.value.trim() && !confirm('当前账号广告内容不为空，确定要用 Markdown 示例覆盖吗？')) return;
            document.getElementById('s_account_format').value = 'markdown';,
          }   account_enable: document.getElementById('s_account_enable').checked ? '1' : '0',
              account_format: document.getElementById('s_account_format').value,
          function fillAccountHtmlExample() {ementById('s_account_content').value,
          ...
          async function saveSettings(btn) {
            try {isabled = true;
              JSON.parse(document.getElementById('s_friends').value);
            } catch (e) { await fetch(ADMIN_PATH + '/api/settings', {
              alert('⚠️ JSON 格式解析错误！请检查是否有遗漏的逗号、引号或括号。');
              return;rs: { 'Content-Type': 'application/json' },
