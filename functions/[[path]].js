...
class NooMiNav {
  ...
  async db_getDashboardData(selected) {
    ...
    const queries = [
      ...
      isDayMode
        ? this.env.db.prepare("SELECT link_id, COUNT(*) as count FROM logs WHERE month_key = ? GROUP BY link_id").bind(currentMonthKey).all().catch(() => ({ results: [] }))
        : Promise.resolve({ results: [] }),
      this.env.db.prepare("SELECT COUNT(*) as total FROM logs WHERE month_key = ?").bind(currentMonthKey).all().catch(() => ({ results: [{ total: 0 }] }))
    ];
...
.glass-card{background:linear-gradient(135deg,rgba(255,255,255,.14),rgba(255,255,255,.08));backdrop-filter:blur(var(--blur));-webkit-backdrop-filter:blur(var(--blur));border:1px solid var(--border);border-radius:20px;box-shadow:var(--shadow-soft);transition:var(--transition)}
...
.section-title{font-size:.95rem;font-weight:800;color:#7dd3fc;margin-bottom:15px;margin-left:6px;text-transform:uppercase;letter-spacing:.06em;text-shadow:0 2px 4px rgba(0,0,0,.35)}
...
.search-box{width:100%;height:56px;padding:0 20px 0 48px;border-radius:18px;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.14);backdrop-filter:blur(8px);color:#fff;font-size:1rem;box-shadow:inset 0 1px 0 rgba(255,255,255,.05),0 4px 14px rgba(0,0,0,.1);transition:var(--transition)}
.search-box::placeholder{color:rgba(255,255,255,.64)}
.search-box:focus{outline:none;background:rgba(255,255,255,.2);border-color:rgba(125,211,252,.4);box-shadow:0 0 0 4px rgba(56,189,248,.1),0 8px 18px rgba(0,0,0,.12)}
.grid-resources{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:18px;margin-bottom:40px}
.resource-card-wrap{display:flex;position:relative;overflow:hidden;min-height:112px;opacity:0;transform:translateY(14px);animation:fadeInUp .5s forwards}
.resource-card-wrap:hover,.partner-card:hover{background:rgba(255,255,255,.22);transform:translateY(-3px);box-shadow:var(--shadow-hover)}
...
.tag-backup{position:absolute;top:12px;right:12px;padding:4px 9px;border-radius:999px;background:rgba(15,23,42,.35);border:1px solid rgba(255,255,255,.12);display:inline-flex;align-items:center;justify-content:center;font-size:11px;color:#e2e8f0;letter-spacing:.02em;text-decoration:none;transition:var(--transition)}
...
.partner-card{text-decoration:none;color:#fff;text-align:center;padding:16px 14px;font-size:.92rem;font-weight:600;border-radius:16px;text-shadow:0 1px 3px rgba(0,0,0,.45);transition:var(--transition);min-height:68px;display:flex;align-items:center;justify-content:center;opacity:0;transform:translateY(14px);animation:fadeInUp .5s forwards}
...
.fab-btn{padding:11px 18px;border-radius:16px;text-decoration:none;font-weight:700;color:#fff;transition:var(--transition);box-shadow:0 6px 16px rgba(0,0,0,.16);white-space:nowrap;display:flex;align-items:center;justify-content:center;text-shadow:0 1px 2px rgba(0,0,0,.28);border:1px solid rgba(255,255,255,.12);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px)}
.fab-telegram{background:rgba(139,92,246,.66)}.fab-mail{background:rgba(59,130,246,.66)}.fab-push{background:rgba(244,63,94,.66)}
.fab-btn:hover{transform:translateY(-2px);box-shadow:0 10px 20px rgba(0,0,0,.2)}
.theme-toggle{position:fixed;top:20px;right:20px;width:44px;height:44px;border-radius:14px;background:rgba(255,255,255,.16);backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,.14);display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:100;color:#fff;font-size:1.1rem;box-shadow:0 6px 16px rgba(0,0,0,.1)}
.no-result{text-align:center;padding:40px 0;color:var(--text-sub);font-size:1.06rem;display:none}
.notice-card{margin-bottom:22px;padding:22px 28px;text-align:left;background:linear-gradient(135deg,rgba(244,63,94,.1) 0%,rgba(30,41,59,.32) 100%);border-left:4px solid #fb7185;animation:fadeInUp .6s forwards;animation-delay:.04s}
...
.promo-card:hover{transform:translateY(-3px);background:linear-gradient(135deg,rgba(255,255,255,.22) 0%,rgba(59,130,246,.16) 100%);box-shadow:0 12px 24px rgba(15,23,42,.16)}
.promo-badge{flex-shrink:0;min-width:138px;padding:12px 16px;border-radius:999px;text-align:center;font-size:.95rem;font-weight:800;line-height:1.35;color:#dbeafe;background:linear-gradient(135deg,rgba(255,255,255,.28),rgba(191,219,254,.18));border:1px solid rgba(255,255,255,.22);box-shadow:inset 0 1px 0 rgba(255,255,255,.18);text-shadow:0 1px 2px rgba(0,0,0,.18)}
.promo-content{display:flex;flex-direction:column;gap:6px;min-width:0}
.promo-title{font-size:1rem;font-weight:800;color:#fff;line-height:1.45;text-shadow:0 2px 6px rgba(0,0,0,.22)}
.promo-desc{font-size:.95rem;color:rgba(226,232,240,.92);line-height:1.6}
.rich-content p{margin:0 0 8px}.rich-content p:last-child{margin-bottom:0}.rich-content h1,.rich-content h2,.rich-content h3{margin:0 0 8px;font-size:1em}.rich-content ul{margin:4px 0 0 18px;padding:0}.rich-content li{margin:4px 0}.rich-content code{padding:2px 6px;border-radius:8px;background:rgba(15,23,42,.25);border:1px solid rgba(255,255,255,.08);font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace;font-size:.9em}.rich-content a{color:#bfdbfe;text-decoration:underline;text-underline-offset:2px}
@keyframes beat{0%,100%{transform:scale(1)}50%{transform:scale(1.22)}}@keyframes fadeInUp{to{opacity:1;transform:translateY(0)}}
@media (max-width:768px){.header h1{font-size:2.2rem}.container{padding:0 10px}.grid-resources{grid-template-columns:1fr;gap:15px}.resource-card-wrap{min-height:106px}.grid-partners{grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:10px}.fab-container{bottom:18px;gap:10px;width:calc(100% - 20px)}.fab-btn{padding:10px 14px;font-size:.85rem}.notice-card{padding:16px 18px}.promo-card{flex-direction:column;align-items:flex-start;gap:14px;padding:18px}.promo-badge{min-width:auto;width:auto;max-width:100%;font-size:.9rem}.promo-title{font-size:.98rem}.promo-desc{font-size:.9rem}}
</style>`;
  }
...
.stat-card{border-radius:20px;border:1px solid var(--bd);background:linear-gradient(180deg,var(--bg-elev),rgba(255,255,255,.02));padding:18px;cursor:pointer;transition:.16s ease;min-height:166px;content-visibility:auto;contain-intrinsic-size:166px}
...
.mini-card{background:linear-gradient(180deg,var(--bg-elev),rgba(255,255,255,.02));border:1px solid var(--bd);border-radius:18px;padding:14px;cursor:pointer;transition:.16s ease;content-visibility:auto;contain-intrinsic-size:90px}
...
.promo-preview-desc p{margin:0 0 8px}.promo-preview-desc p:last-child{margin-bottom:0}.promo-preview-desc ul{margin:4px 0 0 18px;padding:0}.promo-preview-desc li{margin:4px 0}.promo-preview-desc code{padding:2px 6px;border-radius:8px;background:rgba(255,255,255,.06);border:1px solid var(--bd);font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace}.promo-preview-desc a{color:#93c5fd}
...
@media (max-width:768px){body{padding:16px}.quick-stats{grid-template-columns:1fr}.action-row{justify-content:flex-start}.toolbar{padding:14px}.stats-grid{grid-template-columns:1fr}.settings-grid{grid-template-columns:1fr}.drawer{width:100%;right:-100%}.settings-wrap{width:calc(100% - 10px);margin:5px auto;border-radius:20px}.settings-head,.settings-body{padding:16px}.promo-preview-box{flex-direction:column}.promo-preview-badge{min-width:auto;width:fit-content}}
</style>`;
  }
...
document.addEventListener('keydown',e=>{if(e.key==='Escape')cls()});
  ...
  renderResourceCards() {
    return (Array.isArray(this.LINKS_DATA) ? this.LINKS_DATA : []).map(item => {
      ...
      const customTag = item.tag ? `<span class="tag-special">${item.tag}</span>` : "";
      return `<div class="glass-card resource-card-wrap"><a href="/go/${item.id}" class="resource-main-link"><div class="card-icon">${item.emoji || "🔗"}</div><div class="card-info"><h3 style="display:flex;align-items:center;flex-wrap:wrap;">${item.name}${customTag}</h3><p>⚠️ ${item.note || "无说明"}</p></div></a>${backupHtml}</div>`;
    }).join("");
  }
  ...
  renderFab() {
    ...
    if (this.config.contact_url) html += `<a href="${this.config.contact_url}" target="_blank" class="fab-btn fab-telegram">💬 获取支持</a>`;
    if (this.config.mail) html += `<a href="mailto:${this.config.mail}" class="fab-btn fab-mail">📧 发送邮件</a>`;
    if (this.config.push) html += `<a href="/contact" class="fab-btn fab-push">📝 给我留言</a>`;
    return html + `</div>`;
  }

  renderNoticeCard() {
    if (!this.config.notice || !this.config.notice.trim()) return "";
    return `<div class="glass-card notice-card"><div class="notice-title"><span class="heart-beat">❤️</span> 温馨提示</div><div class="notice-content">${this.config.notice}</div></div>`;
  }

  renderPromoCard() {
    if (String(this.config.promo_enable) !== "1" || !this.config.promo_url) return "";
    const desc = this.renderRichContent(this.config.promo_desc, this.config.promo_format);
    return `<a href="${this.config.promo_url}" target="_blank" rel="noopener noreferrer" class="glass-card promo-card"><div class="promo-badge">${this.config.promo_badge || "推广支持"}</div><div class="promo-content"><div class="promo-title">${this.config.promo_title || "推广支持"}</div><div class="promo-desc rich-content">${desc}</div></div></a>`;
  }

  buildAdminCard(id, name, emoji, isMini, dbData, m, viewTotalDenominator) {
    const { statsMap, dailyMap, periodMap, monthContextMap, isDayMode } = dbData;
    ...
    if (isMini) {
      return `<div class="mini-card" onclick="openLog('${id}','${m}','${name}')"><div class="mini-top"><span class="mini-name" title="${name}">${name}</span><span class="mini-badge">${selectedTargetVal}</span></div><div class="mini-meta">${timeDisplay}</div></div>`;
    }

    return `<div class="stat-card" onclick="openLog('${id}','${m}','${name}')"><div class="stat-top"><div class="stat-title-wrap"><span class="stat-emoji">${emoji || "🔗"}</span><span class="stat-title">${name}</span></div><span class="stat-pct">${progressVal}%</span></div><div class="stat-metrics"><div class="metric"><span class="metric-label">历史</span><span class="metric-value">${stat.total_clicks || 0}</span></div><div class="metric"><span class="metric-label">${col2Label}</span><span class="metric-value metric-gold">${col2Val}</span></div><div class="metric"><span class="metric-label">${col3Label}</span><span class="metric-value metric-blue">${col3Val}</span></div></div><div class="progress"><div style="width:${progressVal}%"></div></div><div class="stat-foot">${timeIcon} ${timeDisplay}</div></div>`;
  }

  render_MessageDetail(data) {
    return `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>留言详情</title><style>body{font-family:${this.FONT_STACK};background:#f3f4f6;margin:0;padding:20px;display:flex;justify-content:center;min-height:100vh;box-sizing:border-box}.ticket-card{background:#fff;border-radius:18px;box-shadow:0 10px 30px rgba(0,0,0,.08);width:100%;max-width:600px;padding:40px;margin-top:5vh;height:fit-content;border-top:6px solid #8b5cf6}.header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:25px}.badge{background:#ede9fe;color:#7c3aed;padding:6px 12px;border-radius:20px;font-size:.85rem;font-weight:700;letter-spacing:.5px}.time{color:#94a3b8;font-size:.9rem;font-family:monospace}.sender-box{margin-bottom:25px}.label{font-size:.85rem;color:#64748b;text-transform:uppercase;font-weight:600;letter-spacing:1px;margin-bottom:5px}.sender{font-size:1.5rem;color:#0f172a;font-weight:800;margin:0;word-break:break-all}.divider{height:1px;background:#e2e8f0;margin:25px 0}.message{font-size:1.1rem;line-height:1.8;color:#334155;white-space:pre-wrap;word-break:break-word}.footer{margin-top:40px;text-align:center;color:#cbd5e1;font-size:.85rem}@media (max-width:600px){.ticket-card{padding:25px}.sender{font-size:1.2rem}.message{font-size:1rem}}</style></head><body><div class="ticket-card"><div class="header"><span class="badge">INBOX MESSAGE</span><span class="time">${data.t}</span></div><div class="sender-box"><div class="label">Contact / 发件人</div><h2 class="sender">${data.c}</h2></div><div class="divider"></div><div class="label">Message / 内容</div><div class="message">${data.m}</div><div class="footer">🔒 Encrypted transmission powered by Cloudflare</div></div></body></html>`;
  }

  render_ContactPage() {
    return `<!DOCTYPE html><html><head>${this.render_Head(this.config.title)}<style>.box{padding:40px;width:380px;text-align:left}h2{font-size:1.6rem;margin:0 0 10px;display:flex;align-items:center;gap:8px}p.desc{color:#cbd5e1;font-size:.92rem;margin-bottom:25px;line-height:1.65}form{display:flex;flex-direction:column;width:100%}label{font-size:.85rem;color:#f1f5f9;margin-bottom:8px;font-weight:600}input,textarea{width:100%;padding:14px;background:rgba(0,0,0,.3);border:1px solid rgba(255,255,255,.2);border-radius:14px;color:#fff;margin-bottom:20px;outline:none;transition:.2s;font-size:.95rem;box-sizing:border-box;font-family:inherit}input:focus,textarea:focus{border-color:#60a5fa;background:rgba(0,0,0,.5);box-shadow:0 0 0 4px rgba(96,165,250,.18)}input::placeholder,textarea::placeholder{color:rgba(255,255,255,.4)}textarea{resize:vertical;min-height:100px}button{width:100%;padding:16px;background:linear-gradient(135deg,#3b82f6,#8b5cf6);color:#fff;border:none;border-radius:14px;font-weight:800;cursor:pointer;font-size:1rem;transition:.2s;box-shadow:0 4px 12px rgba(59,130,246,.26)}button:hover{transform:translateY(-2px);box-shadow:0 8px 22px rgba(59,130,246,.35)}button:disabled{opacity:.7;cursor:not-allowed;transform:none}.status{margin-top:15px;font-size:.9rem;font-weight:600;text-align:center;min-height:20px}.back{text-align:center;margin-top:20px}.back a{color:#94a3b8;text-decoration:none;font-size:.85rem;transition:.2s}.back a:hover{color:#fff}</style></head><body style="${this.getBgShellStyle()} display:flex;justify-content:center;align-items:center;min-height:100vh;margin:0"><div class="glass-panel box"><h2>📝 给我留言</h2><p class="desc">有任何问题、疑问？<br>留下联系方式，看到了就会联系，优先邮箱或者QQ。</p><form id="msgForm"><label>留下你的联系方式？</label><input type="text" name="guest_contact" placeholder="邮箱或者QQ" required><label>你想说什么？</label><textarea name="message" placeholder="写下你的留言内容..." required></textarea><button type="submit" id="submitBtn">发送留言</button></form><div id="status" class="status"></div><div class="back"><a href="/">← 返回导航主页</a></div></div>${this.render_BgRuntimeScript()}<script>document.getElementById('msgForm').addEventListener('submit',async e=>{e.preventDefault();const btn=document.getElementById('submitBtn'),status=document.getElementById('status');btn.disabled=true;btn.innerText='发送中...';status.innerText='';try{const res=await fetch('/contact',{method:'POST',body:new FormData(e.target)}),text=await res.text();status.style.color=res.ok?'#34d399':'#f87171';status.innerText=text;if(res.ok)e.target.reset()}catch{status.style.color='#f87171';status.innerText='网络错误，请稍后重试'}finally{btn.disabled=false;btn.innerText='发送留言'}});</script></body></html>`;
  }

  render_LoginPage(errorMsg = "") {
    return `<!DOCTYPE html><html><head>${this.render_Head(this.config.title)}<style>.box{padding:50px 40px;text-align:center;width:340px;display:flex;flex-direction:column;align-items:center}h1{font-size:1.8rem;margin-bottom:30px}form{width:100%;display:flex;flex-direction:column;align-items:center}input{width:100%;padding:16px;background:rgba(0,0,0,.25);border:1px solid rgba(255,255,255,.2);border-radius:14px;color:#fff;margin-bottom:20px;outline:none;transition:.2s;font-size:1rem;box-sizing:border-box;text-align:center}input:focus{border-color:#60a5fa;background:rgba(0,0,0,.5);transform:scale(1.01);box-shadow:0 0 0 4px rgba(96,165,250,.18)}input::placeholder{color:rgba(255,255,255,.5)}button{width:100%;padding:16px;background:linear-gradient(135deg,#3b82f6,#8b5cf6);color:#fff;border:none;border-radius:14px;font-weight:800;cursor:pointer;font-size:1rem;transition:.2s;box-shadow:0 4px 12px rgba(59,130,246,.25)}button:hover{transform:translateY(-2px);box-shadow:0 8px 22px rgba(59,130,246,.35)}.error-msg{color:#f87171;margin-bottom:15px;font-size:.9rem;min-height:20px}</style></head><body style="${this.getBgShellStyle()} display:flex;justify-content:center;align-items:center;min-height:100vh;margin:0"><div class="glass-panel box"><h1>🔐 管理后台</h1>${errorMsg ? `<div class="error-msg">❌ ${errorMsg}</div>` : ""}<form method="POST" action="${this.ADMIN_PATH}"><input type="password" name="password" placeholder="请输入访问口令" required autofocus><button type="submit">立即登录</button></form></div>${this.render_BgRuntimeScript()}</body></html>`;
  }
  ...
  render_AdminDashboard(dbData, m) {
    ...
    try {
      ...
      let ny = cy, nm = cm + 1; if (nm === 13) { ny += 1; nm = 1; }
      prevMonthStr = `${py}_${String(pm).padStart(2, "0")}`;
      nextMonthStr = `${ny}_${String(nm).padStart(2, "0")}`;
    } catch {}

    const linkHtml = safeLinks.map(i => this.buildAdminCard(i.id, i.name, i.emoji, false, dbData, m, viewTotalDenominator)).join("");
    const friendHtml = safeFriends.map(i => this.buildAdminCard(i.id, i.name, "", true, dbData, m, viewTotalDenominator)).join("");
    ...
    const noticeHtml = this.config.notice?.trim()
      ? `<div class="panel notice-panel"><div class="panel-head"><span>❤️</span><strong>公告预览</strong></div><div class="notice-preview">${this.config.notice}</div></div>`
      : "";

    const promoHtml = String(this.config.promo_enable) === "1"
