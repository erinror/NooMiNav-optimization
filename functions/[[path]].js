...
class NooMiNav {
    ...
    async initConfig() {
        ...
        this.config = {
            ...
            // 🟢 动态公告脱敏处理，支持 HTML
            notice: this.dbSettings.notice !== undefined ? this.dbSettings.notice : (this.env.notice || "<div style=\"margin-bottom:8px\">🎉 欢迎使用 FlarePortal 极简导航！</div><div class=\"notice-sub\">您可以在后台「系统设置」中修改此处的公告内容，支持 HTML 标签。如果清空内容，公告板将自动隐藏。</div>")
        };
    ...
    render_AdminDashboard(dbData, m) {
        ...
        const sysSettings = { admin_pass: this.config.admin_pass, title: this.config.title, subtitle: this.config.subtitle, img: this.dbSettings.img || this.env.img || "", contact_url: this.config.contact_url, mail: this.config.mail, push: this.dbSettings.push || this.env.push || "", host: this.dbSettings.host || this.env.host || "", notice: this.config.notice, links: JSON.stringify(this.LINKS_DATA, null, 2), friends: JSON.stringify(this.FRIENDS_DATA, null, 2) };

        let noticeHtmlPreview = '';
        if (this.config.notice && this.config.notice.trim() !== '') {
            noticeHtmlPreview = `<div class="glass-card notice-card"><div class="notice-title"><span class="heart-beat">❤️</span> 温馨提示</div><div class="notice-content">${this.config.notice}</div></div>`;
        }

        return `<!DOCTYPE html><html><head>${this.render_Head(this.config.title)}<style>
          :root { 
              --glass: rgba(15, 23, 42, 0.75); --border: rgba(255, 255, 255, 0.15); 
              --text-main: #f8fafc; --text-sub: #94a3b8; --input-bg: rgba(15, 23, 42, 0.6); 
              --input-border: rgba(255, 255, 255, 0.2); --modal-bg: rgba(15, 23, 42, 0.95); 
              --card-bg: rgba(30, 41, 59, 0.75); --title-shadow: 0 2px 10px rgba(0,0,0,0.3);
              --glow-shadow: 0 0 10px rgba(56,189,248,0.5);
          }
          .light-theme { 
              --glass: rgba(255, 255, 255, 0.9); --border: rgba(0, 0, 0, 0.1); 
              --text-main: #0f172a; --text-sub: #475569; --input-bg: #ffffff; 
              --input-border: #cbd5e1; --modal-bg: rgba(241, 245, 249, 0.98); 
              --card-bg: rgba(255, 255, 255, 0.9); --title-shadow: none; --glow-shadow: none;
          }
          ...
          .fs-modal::-webkit-scrollbar { width: 8px; } .fs-modal::-webkit-scrollbar-track { background: transparent; } .fs-modal::-webkit-scrollbar-thumb { background: rgba(156, 163, 175, 0.4); border-radius: 10px; } .fs-modal::-webkit-scrollbar-thumb:hover { background: rgba(156, 163, 175, 0.7); }
          ...
          .notice-card { margin-bottom: 30px; padding: 22px 28px; text-align: left; background: linear-gradient(135deg, rgba(244, 63, 94, 0.1) 0%, rgba(30, 41, 59, 0.4) 100%); border-left: 4px solid #fb7185; backdrop-filter: blur(20px); animation: fadeInUp 0.8s forwards; animation-delay: 0.05s; } .notice-title { font-size: 1.15rem; font-weight: 800; background: linear-gradient(to right, #fb7185, #c084fc); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 15px; display: flex; align-items: center; gap: 10px; text-shadow: none; } .notice-title span { -webkit-text-fill-color: initial; } .notice-content { font-size: 0.95rem; line-height: 1.8; color: var(--text-main); letter-spacing: 0.5px; } .notice-highlight { color: #fcd34d; font-weight: 700; padding: 0 4px; background: rgba(252, 211, 77, 0.1); border-radius: 4px; } .notice-sub { margin-top: 8px; font-size: 0.9rem; opacity: 0.8; font-style: italic; } .heart-beat { display: inline-block; animation: beat 1.5s infinite ease-in-out; }
          ...
          <div class="fs-modal" id="set-fs">
            ...
            <div class="fs-content">
                <div class="set-grid">
                    ...
                    <div class="set-field set-full"><label>温馨提示公告 (支持 HTML 语法，清空则不显示该卡片)</label><textarea id="s_notice" style="height:120px"></textarea></div>
